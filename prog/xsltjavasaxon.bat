...
if not exist %sjardir%saxon.jar goto :checkprog
goto :ready

:checkprog
set sjardir=..\prog\
if not a%sjardir% == a..\prog\ goto :noenv
if not exist %sjardir%saxon.jar goto :checkprev
goto :ready

:checkprev
set sjardir=%saxon%
if not a%sjardir% == a%saxon% goto :noenv
if not exist %sjardir%saxon.jar goto :nojar

:ready
echo Transforming %1 with %2 to %3 with XSLT 1.0...
java -cp "%sjardir%;%sjardir%saxon.jar" com.icl.saxon.StyleSheet -o %3 %1 %2
REM post-process results
goto :done

:nojar
if not exist ..\%sjardir%saxon.jar echo "saxon.jar" does not exist in "..\", "..\prog" or "%sjardir%"
goto :done
